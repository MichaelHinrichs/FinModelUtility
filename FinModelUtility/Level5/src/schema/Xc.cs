  ...
  public class Xc : IDeserializable {
    ...
    public void Read(EndianBinaryReader er) {
      ...
      for (int i = 0; i < fileCount; i++) {
        ...
        er.Subread(offset, ser => hashToData.Add(nameCrc, ser.ReadBytes((int)size)));
      }

      byte[] nameTable = Array.Empty<byte>();
      er.Subread(fileTableOffset, ser => nameTable = ser.ReadBytes(filenameTableSize));

      if (!Decompress.CheckLevel5Zlib(nameTable, out nameTable)) {
        nameTable = Decompress.DecompressLzss(nameTable);
      }
