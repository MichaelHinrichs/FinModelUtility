...
void main() {
  ...
  vec3 colorComponent = clamp(texture(texture1, uv1).rgb*clamp(clamp((texture(texture0, uv0).rgb + clamp((vec3(1) + vec3(-1)*texture(texture2, acos(normalUv) / 3.14159).rgb), 0, 1)*(vec3(1) + vec3(-1)*vec3(texture(texture0, uv0).a))), 0, 1)*vertexColor0.rgb*clamp((individualLightColors[0].rgb + color_GxAmbientColor0), 0, 1), 0, 1)*vec3(2), 0, 1);
