...
void main() {
  ...
  vec3 colorComponent = clamp((color_GxMaterialColor0*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + color_GxAmbientColor0), 0, 1)*(vec3(1) + vec3(-1)*color_GxColor3) + texture(texture0.sampler, clamp((texture0.transform2d * acos(normalUv) / 3.14159).xy, texture0.clampMin, texture0.clampMax)).rgb*color_GxColor3 + color_GxMaterialColor1*clamp((individualLightDiffuseColors[7].rgb + clamp(color_GxAmbientColor1, 0, 1)), 0, 1))*vec3(2), 0, 1);
