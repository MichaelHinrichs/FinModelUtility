...
void main() {
  ...
  vec3 colorComponent = (vec3(0.501960813999176)*ambientLightColor.rgb + mergedLightDiffuseColor.rgb)*vec3(2)*texture(texture0.sampler, clamp(transformUv3d(texture0.transform3d, uv0), texture0.clampMin, texture0.clampMax)).rgb + mergedLightSpecularColor.rgb;
