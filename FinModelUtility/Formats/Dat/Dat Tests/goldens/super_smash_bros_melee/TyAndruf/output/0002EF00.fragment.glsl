...
void main() {
  ...
  vec3 colorComponent = texture(texture0.sampler, clamp(asin((texture0.transform3d * vec4(normalUv, 0, 1)).xy) / 3.14159 + 0.5, texture0.clampMin, texture0.clampMax)).rgb*(vec3(0.9529411792755127,0.8509804010391235,0) + individualLightColors[0].rgb*vec3(0.7019608020782471,0.501960813999176,0));
