...
void main() {
  ...
  vec3 colorComponent = clamp(clamp(color_GxMaterialColor0*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + color_GxAmbientColor0), 0, 1)*texture(texture1, uv0).rgb*vec3(2), 0, 1) + texture(texture2, uv0).rgb*clamp((color_GxColor0 + texture(texture0.sampler, clamp((texture0.transform2d * acos(normalUv) / 3.14159).xy, texture0.clampMin, texture0.clampMax)).rgb*(vec3(1) + vec3(-1)*texture(texture0.sampler, clamp((texture0.transform2d * acos(normalUv) / 3.14159).xy, texture0.clampMin, texture0.clampMax)).rgb) + texture(texture0.sampler, clamp((texture0.transform2d * acos(normalUv) / 3.14159).xy, texture0.clampMin, texture0.clampMax)).rgb*texture(texture0.sampler, clamp((texture0.transform2d * acos(normalUv) / 3.14159).xy, texture0.clampMin, texture0.clampMax)).rgb)*vec3(2), 0, 1)*color_GxMaterialColor0*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + color_GxAmbientColor0), 0, 1), 0, 1);
