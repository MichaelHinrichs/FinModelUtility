...
namespace HaloWarsTools {
  public class HWXtdResource : HWBinaryResource {
    ...
    public Bitmap AmbientOcclusionTexture =>
        ValueCache.Get(() => ExtractEmbeddedDXT5A(GetFirstChunkOfType(HWBinaryResourceChunkType.XTD_AOChunk)));

    public Bitmap OpacityTexture =>
        ValueCache.Get(() => ExtractEmbeddedDXT5A(GetFirstChunkOfType(HWBinaryResourceChunkType.XTD_AlphaChunk)));
    ...
    private IModel ImportMesh() {
      ...
      Vector3 PosCompRange =
          BinaryUtils.ReadVector3BigEndian(RawBytes, (int) atlasChunk.Offset + 16).ReverseComponents();
        ...
        for (int z = 0; z < gridSize; z += stride) {
          ...
          Vector3 normal =
              ConvertDirectionVector(
                  Vector3.Normalize(ReadVector3Compressed(normalOffset + offset) * 2.0f - Vector3.One));
