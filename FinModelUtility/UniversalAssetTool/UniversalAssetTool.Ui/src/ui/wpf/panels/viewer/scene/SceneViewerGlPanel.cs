...
using uni.model;
using uni.ui.common;
using uni.ui.wpf.common;

namespace uni.ui.wpf.panels.viewer.scene {
  public sealed class SceneViewerGlPanel : BGlPanel {
    ...
    public SceneViewerGlPanel() {
      ...
      this.impl_.MouseMove += (_, args) => {
        if (this.isMouseDown_) {
          var mouseLocation = args.GetPosition(this);
          var mouseX = (int) mouseLocation.X;
          var mouseY = (int) mouseLocation.Y;
          ...
          if (this.prevMousePosition_ != null) {
            ...
            var fovY = this.fovY_;
            var fovX = (float) (fovY / this.Height * this.Width);

            var deltaXFrac = 1f * deltaMouseX / (float) this.Width;
            var deltaYFrac = 1f * deltaMouseY / (float) this.Height;
          ...
          this.prevMousePosition_ = (mouseX, mouseY);
        }
      };
