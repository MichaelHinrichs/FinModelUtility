...
namespace uni.ui.wpf.common {
  public static class BetterTreeUtil {
    public static void ForEach<T, S>(TreeViewItemCollection collection,
                                     Action<T> callback)
        where T : IBetterTreeNode<S>
        where S : class {
      // TODO: Remove unboxing?
      foreach (var childTreeNodeObj in collection) {
        var childTreeNode = (TreeViewItem) childTreeNodeObj;
    ...
    public static T GetBetterFrom<T, S>(TreeViewItem node)
        where T : IBetterTreeNode<S>
        where S : class
      => (T) node.Tag;

    public static T? GetBetterFromNullable<T, S>(TreeViewItem? node)
        where T : IBetterTreeNode<S>
        where S : class
      => (T?) node?.Tag;
  }
  ...
  public class BetterTreeView<T> : IBetterTreeView<T> where T : class {
    ...
    public IBetterTreeNode<T>? SelectedNode
      => BetterTreeUtil.GetBetterFromNullable<IBetterTreeNode<T>, T>(
          this.impl_.SelectedItem as TreeViewItem);
    ...
    public BetterTreeView(TreeView impl) {
      this.impl_ = impl;
      this.Root = new BetterTreeNode(this, null, null, impl.Items);

      this.impl_.SelectedItemChanged += (_, _)
    ...
    public void ScrollToTop() {
      var nodes = this.impl_.Items;
      if (nodes.Count > 0) {
        (nodes[0] as TreeViewItem)!.BringIntoView();
      }
    }
    ...
    private class BetterTreeNode : IBetterTreeNode<T> {
      ...
      private readonly ItemCollection collection_;

      private int openImageIndex_ = -1;
      private int closedImageIndex_ = -1;

      public TreeViewItem? Impl { get; }
    ...
    private class BetterTreeViewComparer : IComparer {
      ...
      public int Compare(object lhs, object rhs) {
        ...
        return comparer.Compare((BetterTreeNode) (((TreeViewItem) lhs).Tag),
                                (BetterTreeNode) (((TreeViewItem) rhs).Tag));
      }
