...
namespace Quad64.graph {
  public class GraphNodeParser : IGraphNodeParser {
    ...
    public IGraphNode Parse(uint address,
      ...
      byte[] data = Assert.Nonnull(rom.getSegment(graphNodeSegment, areaId));

      IGraphNode node = new ParentGraphNode();
      graphReturnType = GraphReturnType.UNDEFINED;

      IF3dDisplayListParser displayListParser = default;
      ...
      while (graphReturnType == GraphReturnType.UNDEFINED) {
        ...
        switch ((GraphCommand) cmd[0]) {
          ...
          case GraphCommand.G_0x05: {
            node = Assert.Nonnull(node.Parent);
            break;
          }
          // 0x08: Viewport
          case GraphCommand.G_0x08: {
            node.AddChildToEnd(new NoopGraphNode("Viewport"));
            break;
          }
          // 0x09: Ortho projection
          case GraphCommand.G_0x09: {
            node.AddChildToEnd(new NoopGraphNode("Ortho projection"));
            break;
          }
          // 0x0A: Camera frustum
          case GraphCommand.G_0x0A: {
            node.AddChildToEnd(new NoopGraphNode("Camera frustum"));
            break;
          }
          // 0x19: Background scene
          case GraphCommand.G_0x19: {
            node.AddChildToEnd(new NoopGraphNode("Background scene"));
            break;
          }
          // 0x1A: Noop
          ...
          // 0x1D: Scale
          case GraphCommand.G_0x1D: {
            var param = cmd[1];
            var scale = cmd[4] / 65536.0f;

            // TODO: Handle drawing layer?

            var displayList = ((param & 0x80) != 0)
                                  ? displayListParser.Parse(
                                      BitLogic.BytesToInt(cmd, 8, 4))
                                  : null;

            node.AddChildToEnd(new ScaleGraphNode {
    ...
    private class ScaleGraphNode : BGraphNode, IScaleGraphNode {
      public IF3dDisplayList? DisplayList { get; init; }
