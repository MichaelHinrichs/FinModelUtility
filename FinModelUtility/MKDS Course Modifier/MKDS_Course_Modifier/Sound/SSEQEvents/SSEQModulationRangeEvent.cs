...
// Type: MKDS_Course_Modifier.Sound.SSEQEvents.SSEQModulationRangeEvent
...
namespace MKDS_Course_Modifier.Sound.SSEQEvents
{
  public class SSEQModulationRangeEvent : SSEQEvent
  {
    ...
    public SSEQModulationRangeEvent(byte EventID, EndianBinaryReader er)
    {
      ...
      this.Range = er.ReadByte();
    }

    public override void AddMidiEvents(ref SSEQMidiResult Result)
    {
      MidiEvent midiEvent = MidiEvent.FromRawMessage(MidiMessage.ChangeControl(77, 64 + (int) this.Range / 2, Result.TrackID + 1).RawData);
