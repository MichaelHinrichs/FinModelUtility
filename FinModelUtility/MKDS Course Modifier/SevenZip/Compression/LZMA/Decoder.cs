  ...
  public class Decoder : ICoder, ISetDecoderProperties
  {
    ...
    private void Init(Stream inStream, Stream outStream)
    {
      ...
      for (uint index1 = 0; index1 < 12U; ++index1)
      {
        for (uint index2 = 0; index2 <= this.m_PosStateMask; ++index2)
        {
          ...
          this.m_IsMatchDecoders[(IntPtr) num].Init();
          this.m_IsRep0LongDecoders[(IntPtr) num].Init();
        }
    ...
    private class LiteralDecoder
    {
      ...
      public void Create(int numPosBits, int numPrevBits)
      {
        ...
        this.m_Coders = new Decoder.LiteralDecoder.Decoder2[(IntPtr) num];
      ...
      private struct Decoder2
      {
        ...
        public byte DecodeNormal(SevenZip.Compression.RangeCoder.Decoder rangeDecoder)
        {
          ...
          do
          {
            num = num << 1 | this.m_Decoders[(IntPtr) num].Decode(rangeDecoder);
          }
          while (num < 256U);
          return (byte) num;
        }
        ...
        public byte DecodeWithMatchByte(SevenZip.Compression.RangeCoder.Decoder rangeDecoder, byte matchByte)
        {
          ...
          do
          {
            ...
            matchByte <<= 1;
            uint num3 = this.m_Decoders[(IntPtr) ((uint) (1 + (int) num2 << 8) + num1)].Decode(rangeDecoder);
            num1 = num1 << 1 | num3;
            if ((int) num2 != (int) num3)
            {
              while (num1 < 256U)
                num1 = num1 << 1 | this.m_Decoders[(IntPtr) num1].Decode(rangeDecoder);
              break;
            }
          }
          while (num1 < 256U);
