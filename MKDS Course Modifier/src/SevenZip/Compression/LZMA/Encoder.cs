...
namespace SevenZip.Compression.LZMA
{
  public class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties
  {
    ...
    private IMatchFinder _matchFinder = (IMatchFinder) null;
    ...
    private void Init()
    {
      ...
      for (uint index1 = 0; index1 < 12U; ++index1)
      {
        for (uint index2 = 0; index2 <= this._posStateMask; ++index2)
        {
          ...
          this._isMatch[(IntPtr) num].Init();
          this._isRep0Long[(IntPtr) num].Init();
        }
    ...
    private uint GetPureRepPrice(uint repIndex, Base.State state, uint posState)
    {
      ...
      if (repIndex == 0U)
      {
        num = this._isRepG0[(IntPtr) state.Index].GetPrice0() + this._isRep0Long[(IntPtr) ((state.Index << 4) + posState)].GetPrice1();
      }
      else
      {
        uint price1 = this._isRepG0[(IntPtr) state.Index].GetPrice1();
        num = repIndex != 1U ? price1 + this._isRepG1[(IntPtr) state.Index].GetPrice1() + this._isRepG2[(IntPtr) state.Index].GetPrice(repIndex - 2U) : price1 + this._isRepG1[(IntPtr) state.Index].GetPrice0();
      }
      return num;
    }
    ...
    private uint Backward(out uint backRes, uint cur)
    {
      ...
      do
      {
        ...
        uint num1 = posPrev;
        uint num2 = backPrev;
        backPrev = this._optimum[(IntPtr) num1].BackPrev;
        posPrev = this._optimum[(IntPtr) num1].PosPrev;
        this._optimum[(IntPtr) num1].BackPrev = num2;
        this._optimum[(IntPtr) num1].PosPrev = cur;
        cur = num1;
      }
      while (cur > 0U);
    ...
    private uint GetOptimum(uint position, out uint backRes)
    {
      ...
      {
        uint num = this._optimum[(IntPtr) this._optimumCurrentIndex].PosPrev - this._optimumCurrentIndex;
      ...
      for (uint index = 0; index < 4U; ++index)
      {
        this.reps[(IntPtr) index] = this._repDistances[(IntPtr) index];
        this.repLens[(IntPtr) index] = this._matchFinder.GetMatchLen(-1, this.reps[(IntPtr) index], 273U);
        if (this.repLens[(IntPtr) index] > this.repLens[(IntPtr) num2])
          num2 = index;
      }
      if (this.repLens[(IntPtr) num2] >= this._numFastBytes)
      {
        backRes = num2;
        uint repLen = this.repLens[(IntPtr) num2];
      ...
      if (lenRes1 < 2U && (int) indexByte1 != (int) indexByte2 && this.repLens[(IntPtr) num2] < 2U)
      {
        backRes = uint.MaxValue;
        return 1;
      }
      ...
      uint num3 = price1 + this._isRep[(IntPtr) this._state.Index].GetPrice1();
      ...
      uint num5 = lenRes1 >= this.repLens[(IntPtr) num2] ? lenRes1 : this.repLens[(IntPtr) num2];
      if (num5 < 2U)
      {
      ...
      {
        uint num4 = 0;
        while (len1 > this._matchDistances[(IntPtr) num4])
          num4 += 2U;
        while (true)
        {
          flag1 = true;
          uint matchDistance = this._matchDistances[(IntPtr) (num4 + 1U)];
          uint num7 = num8 + this.GetPosLenPrice(matchDistance, len1, posState1);
          Encoder.Optimal optimal = this._optimum[(IntPtr) len1];
          if (num7 < optimal.Price)
          {
            optimal.Price = num7;
            optimal.PosPrev = 0U;
          ...
          if ((int) len1 == (int) this._matchDistances[(IntPtr) num4])
          {
            num4 += 2U;
      ...
      while (true)
      {
        ...
        do
        {
          ...
          if ((int) cur != (int) num5)
          {
            ...
            {
              ++position;
              uint num11 = this._optimum[(IntPtr) cur].PosPrev;
              if (this._optimum[(IntPtr) cur].Prev1IsChar)
              {
                --num11;
                if (this._optimum[(IntPtr) cur].Prev2)
                {
                  ...
                  state1 = this._optimum[(IntPtr) num11].State;
                state1.UpdateChar();
              }
              else
                state1 = this._optimum[(IntPtr) num11].State;
              if ((int) num11 == (int) cur - 1)
              {
              ...
              else
              {
                ...
                {
                  num11 = this._optimum[(IntPtr) cur].PosPrev2;
                  num12 = this._optimum[(IntPtr) cur].BackPrev2;
                  state1.UpdateRep();
                }
                else
                {
                  num12 = this._optimum[(IntPtr) cur].BackPrev;
                  if (num12 < 4U)
                    state1.UpdateRep();
                  else
                    state1.UpdateMatch();
                }
                Encoder.Optimal optimal = this._optimum[(IntPtr) num11];
                if (num12 < 4U)
                {
                  switch (num12)
                  {
              ...
              int num13 = (int) price2 + (int) this._isMatch[(IntPtr) ((state1.Index << 4) + posState2)].GetPrice0();
              ...
              num7 = price2 + this._isMatch[(IntPtr) ((state1.Index << 4) + posState2)].GetPrice1();
              num9 = num7 + this._isRep[(IntPtr) state1.Index].GetPrice1();
              if ((int) indexByte4 == (int) indexByte3 && (optimal1.PosPrev >= cur || optimal1.BackPrev != 0U))
              {
                uint num12 = num9 + this.GetRepLen1Price(state1, posState2);
                if (num12 <= optimal1.Price)
                {
                  optimal1.Price = num12;
                  optimal1.PosPrev = cur;
        ...
        Base.State state2;
        if (!flag2 && (int) indexByte4 != (int) indexByte3)
        {
          ...
          if (matchLen >= 2U)
          {
            ...
            uint num11 = num4 + this._isMatch[(IntPtr) ((state2.Index << 4) + posState3)].GetPrice1() + this._isRep[(IntPtr) state2.Index].GetPrice1();
            uint num12 = cur + 1U + matchLen;
            while (num5 < num12)
              this._optimum[(IntPtr) ++num5].Price = 268435455U;
            uint num13 = num11 + this.GetRepPrice(0U, matchLen, state2, posState3);
            Encoder.Optimal optimal = this._optimum[(IntPtr) num12];
        ...
        for (uint repIndex = 0; repIndex < 4U; ++repIndex)
        {
          ...
          if (matchLen1 >= 2U)
          {
            uint num11 = matchLen1;
            do
            {
              while (num5 < cur + matchLen1)
                this._optimum[(IntPtr) ++num5].Price = 268435455U;
              uint num12 = num9 + this.GetRepPrice(repIndex, matchLen1, state1, posState2);
            ...
            if (len2 < num10)
            {
              ...
              uint matchLen2 = this._matchFinder.GetMatchLen((int) len2, this.reps[(IntPtr) repIndex], limit2);
              if (matchLen2 >= 2U)
              {
                state2 = state1;
                state2.UpdateRep();
                uint num12 = position + len2 & this._posStateMask;
                int num13 = (int) num9 + (int) this.GetRepPrice(repIndex, len2, state1, posState2) + (int) this._isMatch[(IntPtr) ((state2.Index << 4) + num12)].GetPrice0();
                subCoder = this._literalEncoder.GetSubCoder(position + len2, this._matchFinder.GetIndexByte((int) len2 - 1 - 1));
                int price = (int) subCoder.GetPrice(true, this._matchFinder.GetIndexByte((int) len2 - 1 - ((int) this.reps[(IntPtr) repIndex] + 1)), this._matchFinder.GetIndexByte((int) len2 - 1));
                uint num15 = (uint) (num13 + price);
                state2.UpdateChar();
                uint posState3 = (uint) ((int) position + (int) len2 + 1) & this._posStateMask;
                uint num16 = num15 + this._isMatch[(IntPtr) ((state2.Index << 4) + posState3)].GetPrice1() + this._isRep[(IntPtr) state2.Index].GetPrice1();
                uint num17 = len2 + 1U + matchLen2;
                while (num5 < cur + num17)
                  this._optimum[(IntPtr) ++num5].Price = 268435455U;
                uint num18 = num16 + this.GetRepPrice(0U, matchLen2, state2, posState3);
                Encoder.Optimal optimal = this._optimum[(IntPtr) (cur + num17)];
        ...
        if (lenRes2 > limit1)
        {
          lenRes2 = limit1;
          uint num11 = 0;
          while (lenRes2 > this._matchDistances[(IntPtr) num11])
            num11 += 2U;
          this._matchDistances[(IntPtr) num11] = lenRes2;
          numDistancePairs = num11 + 2U;
        }
        if (lenRes2 >= num14)
        {
          uint num11 = num7 + this._isRep[(IntPtr) state1.Index].GetPrice0();
          while (num5 < cur + lenRes2)
            this._optimum[(IntPtr) ++num5].Price = 268435455U;
          uint num12 = 0;
          while (num14 > this._matchDistances[(IntPtr) num12])
            num12 += 2U;
          uint len2 = num14;
          while (true)
          {
            flag1 = true;
            uint matchDistance = this._matchDistances[(IntPtr) (num12 + 1U)];
            uint num13 = num11 + this.GetPosLenPrice(matchDistance, len2, posState2);
            Encoder.Optimal optimal1 = this._optimum[(IntPtr) (cur + len2)];
            ...
            if ((int) len2 == (int) this._matchDistances[(IntPtr) num12])
            {
              if (len2 < num10)
              {
                uint limit2 = Math.Min(num10 - 1U - len2, this._numFastBytes);
                uint matchLen = this._matchFinder.GetMatchLen((int) len2, matchDistance, limit2);
                if (matchLen >= 2U)
                {
                  state2 = state1;
                  state2.UpdateMatch();
                  uint num15 = position + len2 & this._posStateMask;
                  int num16 = (int) num13 + (int) this._isMatch[(IntPtr) ((state2.Index << 4) + num15)].GetPrice0();
                  subCoder = this._literalEncoder.GetSubCoder(position + len2, this._matchFinder.GetIndexByte((int) len2 - 1 - 1));
                  int price = (int) subCoder.GetPrice(true, this._matchFinder.GetIndexByte((int) len2 - ((int) matchDistance + 1) - 1), this._matchFinder.GetIndexByte((int) len2 - 1));
                  uint num17 = (uint) (num16 + price);
                  state2.UpdateChar();
                  uint posState3 = (uint) ((int) position + (int) len2 + 1) & this._posStateMask;
                  uint num18 = num17 + this._isMatch[(IntPtr) ((state2.Index << 4) + posState3)].GetPrice1() + this._isRep[(IntPtr) state2.Index].GetPrice1();
                  uint num19 = len2 + 1U + matchLen;
                  while (num5 < cur + num19)
                    this._optimum[(IntPtr) ++num5].Price = 268435455U;
                  uint num20 = num18 + this.GetRepPrice(0U, matchLen, state2, posState3);
                  Encoder.Optimal optimal2 = this._optimum[(IntPtr) (cur + num19)];
                  if (num20 < optimal2.Price)
                  {
              ...
              num12 += 2U;
              if ((int) num12 == (int) numDistancePairs)
                break;
            }
            ++len2;
          }
    ...
    public void CodeOneBlock(out long inSize, out long outSize, out bool finished)
    {
      ...
      else
      {
        while (true)
        {
          ...
          {
            this._isMatch[(IntPtr) num1].Encode(this._rangeEncoder, 0U);
          ...
          {
            this._isMatch[(IntPtr) num1].Encode(this._rangeEncoder, 1U);
            if (backRes < 4U)
            {
              ...
              if (backRes == 0U)
              {
                ...
                if (optimum == 1U)
                  this._isRep0Long[(IntPtr) num1].Encode(this._rangeEncoder, 0U);
                else
                  this._isRep0Long[(IntPtr) num1].Encode(this._rangeEncoder, 1U);
              }
              else
              {
              ...
              if (backRes != 0U)
              {
                for (uint index = backRes; index >= 1U; --index)
                  this._repDistances[(IntPtr) index] = this._repDistances[(IntPtr) (index - 1U)];
          ...
          if (this._additionalOffset == 0U)
          {
            ...
            if (this._matchFinder.GetNumAvailableBytes() != 0U)
            {
              if (this.nowPos64 - nowPos64 >= 4096L)
                goto label_50;
            }
            else
              break;
...
label_50:
    ...
    private class LiteralEncoder
    {
      ...
      public void Create(int numPosBits, int numPrevBits)
      {
        ...
        this.m_Coders = new Encoder.LiteralEncoder.Encoder2[(IntPtr) num];
      ...
      public struct Encoder2
      {
        ...
        public void Encode(SevenZip.Compression.RangeCoder.Encoder rangeEncoder, byte symbol)
        {
          ...
          for (int index = 7; index >= 0; --index)
          {
            ...
            this.m_Encoders[(IntPtr) num].Encode(rangeEncoder, symbol1);
            num = num << 1 | symbol1;
          }
        }
        ...
        public void EncodeMatched(SevenZip.Compression.RangeCoder.Encoder rangeEncoder, byte matchByte, byte symbol)
        {
          ...
          for (int index = 7; index >= 0; --index)
          {
            ...
            this.m_Encoders[(IntPtr) num2].Encode(rangeEncoder, symbol1);
            num1 = num1 << 1 | symbol1;
          }
        }
        ...
        public uint GetPrice(bool matchMode, byte matchByte, byte symbol)
        {
          ...
          if (matchMode)
          {
            for (; num3 >= 0; --num3)
            {
              ...
              num1 += this.m_Encoders[(IntPtr) ((uint) (1 + (int) num4 << 8) + num2)].GetPrice(symbol1);
              num2 = num2 << 1 | symbol1;
          ...
          {
            uint symbol1 = (uint) ((int) symbol >> num3 & 1);
            num1 += this.m_Encoders[(IntPtr) num2].GetPrice(symbol1);
            num2 = num2 << 1 | symbol1;
          }
          return num1;
        }
